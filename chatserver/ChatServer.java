...
public class ChatServer implements Runnable {
	...
	private class MessageHandler {
		...
		private void handleMessage(ByteBuffer messageBuffer, ClientInfo clientInfo) 
				throws IOException, ClassNotFoundException {			
			ServerMessage message = (ServerMessage) ServerMessage.toObject(messageBuffer.array());
	...
	private class ChatProtocol implements Protocol {		
		...
		public void handleMessage(ClientInfo clientInfo, Message<?, ?> message) 
				throws IOException, ClassNotFoundException {
			ChatMessage innerMessage = (ChatMessage) message;
			ChatProtocolKeys actionRequested = innerMessage.getKey();
			System.out.println("chat server recived action Requested: " + actionRequested);
			
			if(actionRequested == ChatProtocolKeys.REGISTRATION_REQUEST) {			
				handleRegistration(innerMessage, clientInfo);
								
			} else if(actionRequested == ChatProtocolKeys.MESSAGE) {
				handleChatMessage(innerMessage, clientInfo);
				
			} else if(actionRequested == ChatProtocolKeys.REMOVE_REQUEST) {
				handleRemoveRequest(clientInfo);
				
			} else {
				sendSingleUserResponseMessage(clientInfo, ChatProtocolKeys.ERROR_MESSAGE, INVALID_KEY_MESSAGE);
			}			
		}		

		private void handleRegistration(ChatMessage innerMessage, ClientInfo clientInfo) throws IOException {
			senderName = innerMessage.getData();
			
			if(isRegistered(clientInfo)) {
				sendSingleUserResponseMessage(clientInfo, ChatProtocolKeys.ERROR_MESSAGE, ALREADY_REGISTERED);
			} 
			else if(!isNameValid(senderName)) {
				sendSingleUserResponseMessage(clientInfo, ChatProtocolKeys.ERROR_MESSAGE, INVALID_NAME);
			} else {
				addChatUser(clientInfo, innerMessage);
				sendSingleUserResponseMessage(clientInfo, ChatProtocolKeys.REGISTRATION_ACK, ACCEPT_MESSAGE);
		...
		private void handleChatMessage(ChatMessage innerMessage, ClientInfo clientInfo) throws IOException {
			if(isRegistered(clientInfo)) {
				senderInfo = findChatClient(clientInfo);
				senderName = senderInfo.getName();
				String messageForUsers = senderName + ": " + innerMessage.getData();
				sendAllUsersExceptOneMessage(senderInfo, ChatProtocolKeys.BROADCAST_MESSAGE, messageForUsers);
			} else {				
				sendSingleUserResponseMessage(clientInfo, ChatProtocolKeys.ERROR_MESSAGE, NOT_REGISTERED);
			}							
		}
		...
		private void handleRemoveRequest(ClientInfo clientInfo) throws IOException {
			if(isRegistered(clientInfo)) {
				...
				sendAllUsersExceptOneMessage(senderInfo, ChatProtocolKeys.REMOVE_REQUEST, senderName + LEAVE_MESSAGE);
			}
		}
		
		private void sendSingleUserResponseMessage(ClientInfo clientInfo, ChatProtocolKeys key, String dataToSend) throws IOException {
			ServerMessage messageForUser = new ServerMessage(PROTOCOL_KEY, new ChatMessage(key, dataToSend));			
		...
		private void sendAllUsersExceptOneMessage(ChatUserInfo senderUserInfo, ChatProtocolKeys key, String dataToSend) throws IOException {
			ServerMessage messageToGroup = new ServerMessage(PROTOCOL_KEY, new ChatMessage(key, dataToSend));
		...
		private ByteBuffer setBuffer(ServerMessage messageToGroup) throws IOException {			
			...
			buffer.clear();
			buffer.put(ServerMessage.toByteArray(messageToGroup));
			buffer.flip();			
			return buffer;
		}
		
		private void addChatUser(ClientInfo clientInfo, ChatMessage innerMessage) {			
